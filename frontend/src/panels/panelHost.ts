 ...
 * whatever was open before it (progressive disclosure: only one primary
 * overlay is meaningfully open at a time). Reacts ONLY to openPanel
 * transitions — uiStore's other fields (provenanceOpenForId) are each
 * panel's own internal concern, already handled by that panel's own
 * subscription; destroying and recreating the whole panel on every such
 * change would both be wasteful and would fight the panel's own internal
 * re-render logic.
 */
