...
/** Mirrors backend/src/types/apiResponse.ts's ApiSuccessResponse<T> — the only success shape this client ever unwraps. */
...
/**
 * Reads and parses the response body as JSON, tolerating a body that
 * isn't valid JSON at all (e.g. a proxy/gateway error page in front of
 * the real API) — never lets a JSON.parse failure surface as an
 * unrelated, confusing error.
 */
 ...
 * taken from the response body, never guessed from the HTTP status
 * alone), and a network-level failure where no response was ever
 * received (NETWORK_ERROR — a client-only code the backend can never
 * produce, since it requires nothing to have reached the server).
 */
...
/**
 * For the export endpoints, whose success body is the raw resource
 * (a GeoJSON FeatureCollection) rather than the `{data}` envelope
 * (export/sendExport.ts's sendGeoJson()) — everything else about
 * request/error handling is identical to requestJson(), so this shares
 * the same fetch/error-normalization path rather than duplicating it.
 */
...
/** For direct-download links (CSV/GeoJSON export) — never fetched programmatically, only used as an <a href>. */
